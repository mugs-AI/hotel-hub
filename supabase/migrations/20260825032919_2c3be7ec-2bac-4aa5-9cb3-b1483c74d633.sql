CREATE INDEX IF NOT EXISTS hotel_reservations_tenant_created_idx
  ON public.hotel_reservations (tenant_id, created_at DESC, id DESC);
CREATE INDEX IF NOT EXISTS hotel_reservation_guests_res_idx
  ON public.hotel_reservation_guests (tenant_id, reservation_id);
CREATE INDEX IF NOT EXISTS hotel_reservation_rooms_res_idx
  ON public.hotel_reservation_rooms (tenant_id, reservation_id);
...
BEGIN
  ...
  CREATE TEMP TABLE IF NOT EXISTS hotelhub_list_tmp () ON COMMIT DROP;
  ...
  keyed AS (
    SELECT a.*,
      CASE v_key
        ...
        WHEN 'roomNo' THEN lower(array_to_string(a.room_labels, ', '))
