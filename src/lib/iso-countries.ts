// ISO 3166-1 country reference data — bundled at build time, no runtime fetch.
// Used for both Nationality and Address Country in guest records.
//
// - `alpha3` is uppercase ISO 3166-1 alpha-3.
// - `name` is the English common name.
// - Malaysia (MYS) is displayed FIRST; every other country is sorted
//   alphabetically by name.

export type Country = { alpha3: string; name: string };
...
const MALAYSIA: Country = { alpha3: "MYS", name: "Malaysia" };

/** Malaysia first, then all remaining countries sorted alphabetically by name. */
export const COUNTRIES: readonly Country[] = (() => {
  const rest = RAW.filter((c) => c.alpha3 !== "MYS").sort((a, b) =>
    a.name.localeCompare(b.name, "en"),
  );
  return [MALAYSIA, ...rest];
})();

const BY_ALPHA3: Map<string, Country> = new Map(COUNTRIES.map((c) => [c.alpha3, c]));

export function isValidCountryCode(v: unknown): v is string {
  return typeof v === "string" && BY_ALPHA3.has(v.toUpperCase());
}

export function normalizeCountryCode(v: unknown): string | null {
  if (typeof v !== "string") return null;
  const u = v.trim().toUpperCase();
  return BY_ALPHA3.has(u) ? u : null;
}

export function countryName(code: string | null | undefined): string {
  if (!code) return "";
  return BY_ALPHA3.get(code.toUpperCase())?.name ?? "";
}

/** Case-insensitive substring match by name or alpha-3 code. */
export function searchCountries(query: string, limit = 50): Country[] {
  const q = query.trim().toLowerCase();
  if (!q) return COUNTRIES.slice(0, limit);
  const out: Country[] = [];
  for (const c of COUNTRIES) {
    if (c.name.toLowerCase().includes(q) || c.alpha3.toLowerCase().includes(q)) {
      out.push(c);
