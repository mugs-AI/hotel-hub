...
/** 128-bit FNV-1a-style hex digest — deterministic, dependency-free. */
function digest(input: string): string {
  ...
  for (let i = 0; i < input.length; i++) {
    ...
    h4 = (Math.imul(h4 ^ c, 668265263) + i) >>> 0;
  }
  return [h1, h2, h3, h4].map((h) => h.toString(16).padStart(8, "0")).join("");
}

/**
 * Canonical, server-generated fingerprint for one reservation mutation.
 *
 * Bound to the reservation ID and stable across browser key order. Raw
 * identity NUMBERS are deliberately excluded (only the action/type are
 * bound) so no secret value can be derived from a stored fingerprint.
 */
export function canonicalFingerprint(
  ...
  const shaped = {
    ...
    guests: input.guests.map((g) => ({
      ...
      // identityNumber intentionally omitted — never fingerprinted.
    })),
  };
  return `hhv2:${digest(canonicalJson(shaped))}`;
}

/** Stable HTTP status for each allow-listed full-update error code. */
